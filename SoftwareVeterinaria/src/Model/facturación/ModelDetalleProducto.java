...
public class ModelDetalleProducto extends DetalleProducto {
    ...
    public boolean CrearDetalleProd(){
        try {
            sql="INSERT INTO DETALLE_PRODUCTO (id_detalle_p,id_producto_d,id_factura_d,cantidad_p,total_detalle_producto,habilidato )";
            sql+="VALUES (?,?,?,?,?,?)";
    ...
    public String codigoDetalle(){
        try {
            ...
            if (rs.next()) {
                String idproducto = rs.getString("id_detalle_p");
                return idproducto;
            }
            rs.close();
        } catch (SQLException ex) {
